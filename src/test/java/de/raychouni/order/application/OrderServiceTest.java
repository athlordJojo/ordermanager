...
class OrderServiceTest {
    ...
    @Mock LoadOrdersOfCompanyPort loadOrdersOfCompanyPort;

    @Mock DeleteOrderOfCompanyPort deleteOrderOfCompanyPort;

    @Mock CreateOrderPort createOrderPort;

    @Mock OrderChangedPort orderChangedPort;

    @Mock LoadCompanyByIdPort loadCompanyByIdPort;

    @Mock LoadOrderOfCompanyPort loadOrderOfCompanyPort;

    @Mock UpdateOrderOfCompanyPort updateOrderOfCompanyPort;
    ...
    void getAllOrdersByCompanyId() {
        ...
        List<Order> ordersByCompanyId =
                orderService.getAllOrdersByCompanyId(new GetAllOrdersForCompanyCommand(company.getUuid()));
    ...
    void createOrder_withNonExistingCompanyId_expectException() {
        doThrow(new EntityNotFoundException())
                .when(loadCompanyByIdPort)
                .loadCompanyById(company.getUuid());
        assertThrows(
                EntityNotFoundException.class,
                () -> {
                    orderService.createOrder(
                            new CreateOrderForCompanyCommand(company.getUuid(), order.getScoreBoardNumber()));
                });
    ...
    void createOrder_withExistingCompanyId_expectSuccess() {
        ...
        Order savedNewOrder =
                orderService.createOrder(
                        new CreateOrderForCompanyCommand(
                                company.getUuid(), orderWithoutId.getScoreBoardNumber()));
    ...
    //        when(orderRepository.findFirstByUuidAndCompany_Uuid(order.getUuid(),
    // company.getUuid())).thenReturn(Optional.empty());
    //        assertThrows(EntityNotFoundException.class, () -> {
    //            orderService.updateOrder(new OrderJPA(), company.getUuid(), order.getUuid());
    //        });
    //        verify(orderRepository).findFirstByUuidAndCompany_Uuid(order.getUuid(),
    // company.getUuid());
    ...
    //
    //        when(orderRepository.findFirstByUuidAndCompany_Uuid(order.getUuid(),
    // company.getUuid())).thenReturn(Optional.of(order));
    ...
    //
    //        OrderJPA updatedOrder = orderService.updateOrder(changeSetOrder, company.getUuid(),
    // this.order.getUuid());
    //
    ...
    //
    //        verify(orderRepository).findFirstByUuidAndCompany_Uuid(this.order.getUuid(),
    // company.getUuid());
    ...
    void deleteOrder_withNonExistingCompanyOrderCombination_expectException() {
        doThrow(new EntityNotFoundException())
                .when(deleteOrderOfCompanyPort)
                .deleteOrderOfCompany(company.getUuid(), order.getUuid());
        assertThrows(
                EntityNotFoundException.class,
                () -> {
                    orderService.deleteOrderOfCompany(
                            new DeleteOrderOfCompanyCommand(company.getUuid(), order.getUuid()));
                });
    ...
    void deleteOrder_withExistingCompanyOrderCombination_expectSuccess() {
        when(deleteOrderOfCompanyPort.deleteOrderOfCompany(company.getUuid(), order.getUuid()))
                .thenReturn(order);
