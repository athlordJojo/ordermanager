...
class OrderControllerTest {
    ...
    void getAllOrders() throws Exception {
        when(getAllOrdersForCompanyUseCase.getAllOrdersByCompanyId(any(GetAllOrdersForCompanyCommand.class))).thenReturn(List.of(order1, order2));

        String expectedResult = objectMapper.writeValueAsString(List.of(getOrderDtoFromOrder(order1), getOrderDtoFromOrder(order2)));

        MockHttpServletRequestBuilder requestBuilder = get(orderUrl)
        ...
        verify(getAllOrdersForCompanyUseCase).getAllOrdersByCompanyId(eq(new GetAllOrdersForCompanyCommand(companyId)));
    ...
    void createOrder() throws Exception {
        ...
        MockHttpServletRequestBuilder requestBuilder = post(orderUrl)
        ...
        verify(createOrderForCompanyUsecase).createOrder(eq(new CreateOrderForCompanyCommand(companyId, scoreBoardNumber)));
        verify(updateOrderOfCompanyUseCase, never()).updateOrder(any());
        verify(deleteOrderOfCompanyUseCase, never()).deleteOrderOfCompany(any());
    }
    ...
    void updateOrder() throws Exception {
        ...
        orderDto.setScoreBoardNumber(scoreBoardNumber);
        MockHttpServletRequestBuilder requestBuilder = put(orderUrl + "/" + order1.getUuid().toString())
                .content(objectMapper.writeValueAsString(orderDto))
                .contentType(APPLICATION_JSON)
                .accept(APPLICATION_JSON);
        ...
        verify(getAllOrdersForCompanyUseCase, never()).getAllOrdersByCompanyId(any());
        verify(createOrderForCompanyUsecase, never()).createOrder(any());
        verify(updateOrderOfCompanyUseCase).updateOrder(eq(new UpdateOrderOfCompanyCommand(companyId, order1.getUuid(), 1, Order.State.IN_PROGRESS, "New Title")));
        verify(deleteOrderOfCompanyUseCase, never()).deleteOrderOfCompany(any());
    }

    @Test
    void deleteOrder() throws Exception {
        MockHttpServletRequestBuilder requestBuilder = delete(orderUrl + "/" + order1.getUuid().toString());
        ...
        verify(createOrderForCompanyUsecase, never()).createOrder(any());
        verify(updateOrderOfCompanyUseCase, never()).updateOrder(any());
        verify(deleteOrderOfCompanyUseCase).deleteOrderOfCompany(eq(new DeleteOrderOfCompanyCommand(companyId, order1.getUuid())));
